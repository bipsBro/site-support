...
class Menu extends \yii\widgets\Menu
{
    ...
    public $submenuTemplate = "\n<ul class='collapse nav' style='padding-left:20px' id='{url}' {show}>\n{items}\n</ul>\n";
    public $activateParents = true;
    public $defaultIconHtml = '<i class="material-icons">dashboard</i> ';
    ...
    protected function renderItem($item)
    {
        if (isset($item['items'])) {
            ...
            $linkTemplate = '<a href="#{url}" data-toggle="collapse"  aria-expanded="false" aria-controls="{terget}" >{icon} {label}</a>';
        } else {
        ...
        $replacements = [
            ...
            '{terget}' => isset($item['terget']) ? Url::to($item['terget']) : 'javascript:void(0);',
        ];
    ...
    {
       // echo '<hr>', '-', $item['url'];
        if (isset($item['url']) or is_array($item['url']) or isset($item['url'][0])) {
            $route = $item['url'][0];
